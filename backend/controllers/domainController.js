  ...
  try {
    ...
    if (!apiKey) {
      ...
      return res.status(500).json({
        success: false,
        message: "Missing API key in backend configuration",
      });
    }

    const url = `https://domain-availability.whoisxmlapi.com/api/v1?apiKey=${apiKey}&domainName=${encodeURIComponent(
      domain
    )}&outputFormat=JSON`;
