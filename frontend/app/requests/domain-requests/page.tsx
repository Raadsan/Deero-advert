...
export default function DomainRequestsPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const domainRequests = allTransactions
        .filter((t: any) => t.type === "register")
        .reverse();
      setRows(domainRequests);
    } catch (err) {
  ...
  const columns = [
    { label: "Domain Name", key: "domainName", width: "200px" },
    { 
        label: "User", 
        key: "user", 
        render: (r: any) => (
  ...
  return (
    <div className="space-y-6">
      <DataTable title="Domain Requests" columns={columns} data={rows} onRefresh={fetchData} loading={loading} />
      <DeleteConfirmModal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} onConfirm={confirmDelete} itemName="this domain request" />
    </div>
  );
}
