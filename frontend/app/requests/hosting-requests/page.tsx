...
export default function HostingRequestsPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const hostingRequests = allTransactions
        .filter((t: any) => t.type === "hosting_payment")
        .reverse();
      setRows(hostingRequests);
    } catch (err) {
      console.error("Failed to load hosting requests", err);
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const columns = [
    { label: "Hosting Package", key: "hostingPackage", width: "200px", render: (r: any) => r.hostingPackage?.name || "N/A" },
    { 
        label: "User", 
        key: "user", 
        render: (r: any) => (
  ...
  return (
    <div className="space-y-6">
      <DataTable title="Hosting Requests" columns={columns} data={rows} onRefresh={fetchData} loading={loading} />
      <DeleteConfirmModal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} onConfirm={confirmDelete} itemName="this hosting request" />
    </div>
  );
}
